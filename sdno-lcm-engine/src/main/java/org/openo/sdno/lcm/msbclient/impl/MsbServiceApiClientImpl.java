...
public class MsbServiceApiClientImpl implements MsbServiceApiClient {
    ...
    public MicroServiceFullInfo register() {
        ...
        Node node = new Node();
        node.setPort(env.getProperty("server.port"));
        node.setIp(env.getProperty("sdno-lcm.ip"));
        List<Node> nodes = new ArrayList<Node>();
        nodes.add(node);
        MicroServiceInfo microServiceInfo = new MicroServiceInfo();
        microServiceInfo.setUrl(env.getProperty("server.contextPath")); // ?
        microServiceInfo.setProtocol(ProtocolEnum.REST);
        microServiceInfo.setServiceName("sdnonslcm"); // ?
        ...
        try {
            ...
            MicroServiceFullInfo registration = getMSBServiceResourceApi().addMicroService(microServiceInfo, false, null);
